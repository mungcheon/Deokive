...
def main() -> int:
    ...
    parser.add_argument(
        "--core-only",
        action="store_true",
        help="Check only reports regenerated by the public catalog update workflow.",
    )
    ...
    field_batches = (
        {"queue_rows": int(field_queue.get("missing_total") or 0)}
        if args.core_only
        else _read_json(args.field_batches)
    )
    image_batches = (
        {"missing_images": int(image_queue.get("missing_images") or 0)}
        if args.core_only
        else _read_json(args.image_batches)
    )
    ...
    report = build_report(
        ...
        image_queue,
        image_batches,
        None if args.core_only else _read_json(args.image_batch_plan),
        None if args.core_only else _read_json(args.image_provider_coverage),
        _read_json(args.source_discovery),
        _read_json(args.stale_source_cleanup) if args.stale_source_cleanup.exists() else None,
        None
        if args.core_only
        else _read_json(args.agent_image_candidates)
        if args.agent_image_candidates.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.agent_image_candidates_broad)
        if args.agent_image_candidates_broad.exists()
        else None,
        None if args.core_only else _read_json(args.db_sync) if args.db_sync.exists() else None,
        None
        if args.core_only
        else _read_json(args.barcode_applicability)
        if args.barcode_applicability.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.metadata_applicability)
        if args.metadata_applicability.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.source_image_applicability)
        if args.source_image_applicability.exists()
        else None,
        None
        if args.core_only
        ...
        if args.source_bottlenecks.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.prize_provider_fallback)
        if args.prize_provider_fallback.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.focus_missing_images)
        if args.focus_missing_images.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.focus_image_template)
        if args.focus_image_template.exists()
        else None,
        None
        if args.core_only
        ...
        if args.ichiban_history_status.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.ichiban_metadata)
        if args.ichiban_metadata.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.ichiban_campaign_gap)
        if args.ichiban_campaign_gap.exists()
        else None,
        None
        if args.core_only
        else _read_json(args.ichiban_prize_structure)
