...
class UpdateWorkflowIntegrityTests(unittest.TestCase):
    ...
    def test_refresh_regenerates_current_queues_before_backlog_and_consistency_audit(self) -> None:
        ...
        field_work_packs = calls.index("tools/build_field_update_work_packs.py")
        ...
        image_work_packs = calls.index("tools/build_image_update_work_packs.py")
        ...
        animation_enrichment = calls.index("tools/build_animation_enrichment_priority_queue.py")
        backlog = calls.index("tools/build_catalog_update_backlog.py")
        operations_dashboard = calls.index("tools/build_catalog_operations_dashboard.py")
        work_pack_coverage = calls.index("tools/audit_catalog_work_pack_coverage.py")
        ...
        self.assertLess(field_queue, field_work_packs)
        self.assertLess(field_work_packs, naming_queue)
        ...
        self.assertLess(image_queue, image_work_packs)
        self.assertLess(image_work_packs, source_discovery)
        self.assertLess(image_queue, source_discovery)
        self.assertLess(source_discovery, missing_image_sync)
        self.assertLess(missing_image_sync, animation_enrichment)
        self.assertLess(animation_enrichment, backlog)
        self.assertLess(missing_image_sync, backlog)
        self.assertLess(backlog, operations_dashboard)
        self.assertLess(operations_dashboard, work_pack_coverage)
        self.assertLess(work_pack_coverage, consistency)
        self.assertLess(operations_dashboard, consistency)
        self.assertLess(consistency, seed)
        self.assertIn("python tools/sync_missing_image_work_queue_public.py --write", text)
        self.assertIn("python tools/build_animation_enrichment_priority_queue.py", text)
        self.assertIn("python tools/build_catalog_operations_dashboard.py", text)
        self.assertIn("python tools/build_field_update_work_packs.py", text)
        self.assertIn("python tools/audit_catalog_work_pack_coverage.py", text)
