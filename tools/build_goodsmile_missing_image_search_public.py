...
def missing_goodsmile_rows(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        item
        for item in items
        if item.get("source_store") == GOODSMILE_STORE and not present(item.get("image_url"))
    ]


def goodsmile_queue_rows(queue: dict[str, Any]) -> list[dict[str, Any]]:
...
def build_report(
    ...
    *,
    ...
    rows = missing_goodsmile_rows(catalog_items(catalog))
    qrows = goodsmile_queue_rows(queue)
    ...
    return {
        ...
        "scope": "goodsmile_missing_image_search",
        "summary": {
            "missing_goodsmile_image_rows": len(rows),
        ...
        "automation_policy": {
            ...
            "requires_exact_goodsmile_product_page": True,
