...
DEFAULT_OUTPUT = DATA / "catalog_manual_source_url_search_queue_public.json"
...
def _priority_score(row: dict[str, Any]) -> tuple[int, int, str]:
    category = _compact_text(row.get("category"))
    category_rank = {
        "인형": 0,
        "아크릴 스탠드": 1,
        "액세서리": 2,
    ...
    return (category_rank, int(row.get("catalog_index") or 999_999_999), _compact_text(row.get("name_ko")))
...
def _queue_item(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "priority_bucket": _compact_text(row.get("category")) or "uncategorized",
        "auto_apply_enabled": False,
    }


def _build_category_workstreams(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    by_category: dict[str, list[dict[str, Any]]] = {}
    for item in items:
        by_category.setdefault(_compact_text(item.get("category")) or "uncategorized", []).append(item)

    workstreams: list[dict[str, Any]] = []
    for category, rows in by_category.items():
        sorted_rows = sorted(rows, key=_priority_score)
        workstreams.append(
            {
                "category": category,
                "row_count": len(sorted_rows),
                "by_source_store": _counter_pairs(sorted_rows, "source_store"),
                "store_search_url_count": sum(1 for row in sorted_rows if row.get("store_search_url")),
                "sample_rows": sorted_rows[:10],
                "recommended_review_order": [
                    "Open store_search_url when it is available.",
                    "Use fallback_search_queries if the store search page has no exact result.",
                    "Confirm the exact product/detail page and variant before importing.",
                    "Fill source_url_import_template.manual_value and set manual_confirmed=true only after review.",
                ],
    ...
    workstreams.sort(key=lambda row: (-int(row["row_count"]), str(row["category"])))
    return workstreams
...
def _review_readiness(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "status": "manual_search_required" if rows else "empty",
        ...
        "blocked_reason": "no_exact_product_candidate" if rows else None,
        "blocked_until": "manual_exact_product_source_url_found" if rows else None,
        "required_evidence": [
...
def build_queue(template: dict[str, Any], *, generated_at: str | None = None) -> dict[str, Any]:
    rows = [
        ...
        if isinstance(row, dict) and row.get("source_url_review_lane") == "manual_search_required"
    ]
    rows.sort(key=_priority_score)
    workstreams = _build_category_workstreams(rows)

    return {
        "schema_version": 1,
        "generated_at": generated_at or now_utc(),
        "scope": "catalog_manual_source_url_search_queue",
        "summary": {
            "manual_search_required_rows": len(rows),
