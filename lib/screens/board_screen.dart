...
class _BoardScreenState extends State<BoardScreen> {
          ...
          body: SafeArea(
            child: Column(
              children: [
                ...
                Expanded(
                  child: _buildTabBody(sections, strings, palette, appState),
                ),
              ],
            ),
          ),
  ...
  Widget _buildTabBody(
    List<_BoardTab> sections,
    AppStrings strings,
    DeokivePalette palette,
    AppState appState,
  ) {
    switch (_currentTab) {
      case 0:
      ...
      case 2:
        return _EventScheduleView(accent: palette.primary);
    }
...
class _FreeTalkViewState extends State<_FreeTalkView> {
  ...
  void _resetTags() => setState(() => _hiddenTags.clear());
...
class _BoardPostCard extends StatelessWidget {
  ...
  const _BoardPostCard({required this.post, required this.isAdmin});
...
class _PostArchiveView extends StatefulWidget {
  final Color accent;
  const _PostArchiveView({required this.accent});
...
class _EventScheduleView extends StatefulWidget {
  ...
  const _EventScheduleView({required this.accent});
...
class _BoardPostDetailScreenState extends State<_BoardPostDetailScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => _maybeTranslate());
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (post.imageBytes != null) const SizedBox(height: 14),
          Text(
            content,
