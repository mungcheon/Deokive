...
class _ImageCatalogSearchScreenState extends State<ImageCatalogSearchScreen> {
  ...
  Future<void> _pickAndSearch(ImageSource source) async {
    ...
    // Full ML Kit pipeline: object detection (crop to subject) + image
    // labeling (narrow candidate pool by product category).
    final analyzed = await svc.analyzeUserPhoto(bytes);
    if (analyzed == null) {
      if (!mounted) return;
      setState(() => _searching = false);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('이미지를 처리하지 못했어요.')),
      );
      return;
    }
